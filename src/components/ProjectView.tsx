...
import { ArrowLeft, Upload, Plus, Download, FileText, Search, Trash2, Minus, ChevronLeft, ChevronRight, Menu } from 'lucide-react';
import { Category, ComponentRef } from '../types';
...
export function ProjectView() {
  const { currentProjectId, currentProject, closeProject, bomLines, manufacturers, addOrUpdateBOMLine, removeBOMLine, updateBOMLineQte, importBOMData, sublists, addSublist, removeSublist } = useStore();
  const fileInputRef = useRef<HTMLInputElement>(null);

  const [activeView, setActiveView] = useState('Globale');
  const projectSublists = sublists.filter(s => s.projectId === currentProjectId);
  ...
  const viewLines = useMemo(() => {
    ...
    let finalFiltered: (typeof filtered[0] & { listsInfo: string[] })[] = [];
    
    if (activeView === 'Globale' || activeView === 'EtatPrepa') {
      const mergedMap = new Map<string, typeof filtered[0] & { listsInfo: string[] }>();
      for (const line of filtered) {
        if (mergedMap.has(line.ref)) {
           ...
           const slName = projectSublists.find(s => s.id === line.sublistId)?.name || 'Inconnu';
           mergedMap.set(line.ref, { ...line, listsInfo: [slName] });
        }
      }
  ...
  }, [bomLines, currentProjectId, activeView, searchTerm, refDetails, manufacturers]);
           ...
           {isSidebarOpen && (
             <div className="flex items-center gap-3 overflow-hidden">
               ...
               <div className="min-w-0">
                 <h2 className="font-bold text-slate-800 text-sm truncate">Affaire {project.id}</h2>
                 <p className="text-xs text-slate-500 truncate">{project.techName}</p>
               </div>
             </div>
           )}
           <button 
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0">
           ...
           <div className="flex items-center gap-3">
              <button 
                onClick={() => ExportService.exportToPDF(project, viewLines, activeView)}
              ...
              <button 
                onClick={() => ExportService.exportToExcel(project, viewLines, activeView)}
          ...
          <div className="flex-1 overflow-auto">
            <table className="min-w-full divide-y divide-slate-200 relative">
              ...
              <tbody className="bg-white divide-y divide-slate-200">
                {viewLines.length === 0 ? (
                  <tr>
                    <td colSpan={(activeView === 'Globale' || activeView === 'EtatPrepa') ? 6 : 5} className="px-6 py-12 text-center text-slate-500 font-medium">
                  ...
                  <tr key={line.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-2.5 whitespace-nowrap text-sm text-slate-900 text-center font-medium bg-blue-50/50">
                    ...
                    <td className="px-6 py-2.5 whitespace-nowrap text-sm text-slate-500 font-mono">{line.fabCode}</td>
                    {(activeView === 'Globale' || activeView === 'EtatPrepa') && (
                      <td className="px-6 py-2.5 whitespace-nowrap text-sm text-slate-600">
                        <div className="flex flex-wrap gap-1">
                          {line.listsInfo.map((cat) => (
                            <span key={cat} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-slate-100 text-slate-800">
                              {cat}
                            </span>
                          ))}
                        </div>
                ...
                <input 
                  ...
                  onKeyDown={e => {
                    if (e.key === 'Enter' && addListModal.name.trim()) {
                      e.preventDefault();
                      addSublist({ projectId: currentProjectId!, name: addListModal.name.trim(), type: addListModal.type });
